...
N <- nrow(eth)
Eth_Close <- eth$Close[N:1]  # reversing the order of datapoints
plot(Eth_Close, type = 'l')

# Stabilizing the variance by transforming the differenced data and visualizing them
plot(diff(Eth_Close), type = 'l', main = 'Original data')
...
# Choosing the random component for further ARIMA analysis of data
...
# Computing AIC table to choose suitable values of p and q
eth_aic_table <- aic_table(eth_close_rand, 6, 6)

# Forming the table with html components
...
# predicting through trained model and forecast the
