...
eth$eth_price_binary_label <- as.factor(eth$eth_price_binary_label) # For converting binary label into a factor 
...
# Creating folds for carrying out  k fold cross validation
set.seed(120)
cv_splits <- createFolds(classes_vec, k=10, returnTrain = TRUE)
#str(cv_splits)

# Number of pca components selected from elbow or scree plots
nPca <- 6
eth_pca <- eth[, 1:nPca]
eth_pca <- cbind(eth_pca, eth_price_binary_label = eth$eth_price_binary_label) # combining the label column into projected dataset
...
# Testing Accuracy was found to be 52.2%. The plot shows the “accuracy”, 
# that is the percentage of correctly classified observations,
